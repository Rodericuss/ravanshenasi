...
```elixir
# Standard pattern in every context (slice 1 example)
...
```elixir
# intended signature: raw result, without {:ok, _} wrapping
...
```elixir
# in priv/repo/migrations: shared function
...
```
lib/ravanshenasi/
  accounts.ex                 # context: solo/clinic registration, invitations, members, scope
  accounts/
    scope.ex                  # Scope: {user, tenant, role} + admin?/1, therapist?/1
    user.ex                   # +tenant_id, +role, +name
    user_token.ex             # (generated)
    user_notifier.ex          # (generated) + invitation email
    tenant.ex
    invitation.ex
  repo.ex                     # +transact_tenant/2, +with_auth_bypass/1, +with_registration_bypass/1
lib/ravanshenasi_web/
  user_auth.ex                # plug + on_mount, extended with tenant in Scope
  live/
    user_*                    # login, registration, settings (generated/extended)
    org/                      # clinic registration, member management, invitations
priv/repo/migrations/
  *_create_tenants.exs
  *_create_users_auth_tables.exs   # generated by phx.gen.auth, + tenant_id/role/name
  *_create_invitations.exs
  *_enable_tenant_rls.exs          # ENABLE/FORCE RLS: users, invitations (tenant_id) + tenants (id); users_tokens stays out
```
