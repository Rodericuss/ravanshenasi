defmodule Ravanshenasi.FrameworksTest do
  ...
  test "admin cria framework de catálogo (user_id nil) e aparece na listagem" do
    ...
    assert {:ok, fw} =
             Frameworks.create_tenant_framework(scope, %{name: "Sistêmica", description: "..."})
  ...
  test "therapist não cria framework de catálogo" do
    ...
    assert {:error, :unauthorized} =
             Frameworks.create_tenant_framework(scope, %{name: "X", description: "y"})
  end
  ...
  test "clinic-admin não cria framework próprio" do
    ...
    assert {:error, :unauthorized} =
             Frameworks.create_own_framework(scope, %{name: "X", description: "y"})
  end
  ...
  test "list_frameworks faz união simples: catálogo do tenant + próprios do user, não de outro user" do
    ...
    {:ok, _cat} =
      Frameworks.create_tenant_framework(admin, %{name: "Catálogo Casa", description: "..."})

    {:ok, _own} =
      Frameworks.create_own_framework(scope, %{name: "Minha Linha", description: "..."})

    {:ok, _other_own} =
      Frameworks.create_own_framework(other, %{name: "Linha do Outro", description: "..."})

    names = Frameworks.list_frameworks(scope) |> Enum.map(& &1.name)
    # herdado do tenant (user_id NULL)
    assert "Catálogo Casa" in names
    # própria
    assert "Minha Linha" in names
